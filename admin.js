...
window.exportProductsCSV=()=>{
  ...
  const headers=['id','name','category','brand','type','shortDesc','desc','originalPrice','discount','stock','sku','weight','images','sizes','colors','material','specs','featured','inStock','visible','freeShipping','badge','tags'];
...
window.importProductsCSV=()=>$('csvFileInput')?.click();
...
$('csvFileInput')?.addEventListener('change',async(e)=>{
  ...
  for(let i=1;i<lines.length;i++){
    ...
    try{
      // Parse arrays
      ['images','sizes','colors','tags'].forEach(k=>{if(obj[k]) obj[k]=obj[k].split(';').filter(Boolean); else obj[k]=[];});
      if(obj.specs){try{obj.specs=JSON.parse(obj.specs);}catch{obj.specs={};}}
      // Parse numbers
      ['originalPrice','discount','stock','weight'].forEach(k=>{if(obj[k]) obj[k]=parseFloat(obj[k]); else obj[k]=null;});
      // Parse booleans
      ['featured','inStock','visible','freeShipping'].forEach(k=>{obj[k]=obj[k]==='true'||obj[k]==='1';});
      obj.createdAt=FB.serverTimestamp();
      await FB.addDoc(FB.collection(FB.db,'products'),obj);
      imported++;
    }catch(err){errors++; console.warn('Import row error:',err);}
  }
  showToast(`Imported ${imported} products${errors?`, ${errors} failed`:''}`, errors?'error':'success');
  $('csvFileInput').value='';
});

window.bulkDeleteVisible=async()=>{
  const body=$('tableBody');
  if(!body) return;
  const rows=body.querySelectorAll('.table-row');
  const visibleIds=[...rows].map(r=>{
    const editBtn=r.querySelector('.tbl-btn.edit');
    if(editBtn) return editBtn.getAttribute('onclick').match(/'([^']+)'/)[1];
    return null;
  }).filter(Boolean);
  if(!visibleIds.length){showToast('No products in current view','error');return;}
  if(!confirm(`Delete ${visibleIds.length} visible products? This cannot be undone.`)) return;
  let deleted=0;
  for(const id of visibleIds){
    try{await FB.deleteDoc(FB.doc(FB.db,'products',id)); deleted++;}catch(e){}
  }
  showToast(`Deleted ${deleted} products`, deleted?'success':'error');
};

/* ─── HERO PREVIEW ─── */
window.previewHero=()=>{
  // Save current values to localStorage for preview
  const heroData={
    eyebrow:$('hEyebrow')?.value||'',
    title:$('hTitle')?.value||'',
    subtitle:$('hSubtitle')?.value||'',
    btnText:$('hBtnText')?.value||'Shop Now',
    btn2Text:$('hBtn2Text')?.value||'',
    btn2Link:$('hBtn2Link')?.value||'',
    bgColor:$('hBgColor')?.value||'#0f0e0d',
    accentColor:$('hAccentColor')?.value||'#e63950',
    showStats:$('hShowStats')?.checked||false,
  };
  localStorage.setItem('heroPreview',JSON.stringify(heroData));
  // Open store in new tab
  window.open('index.html?preview=hero','_blank');
};

window.resetHeroForm=()=>{
  $('hEyebrow').value='Welcome to';
  $('hTitle').value='3mi Store';
  $('hSubtitle').value='Discover curated products at unbeatable prices.';
  $('hBtnText').value='Shop Now';
  $('hBtn2Text').value='';
  $('hBtn2Link').value='';
  $('hBgColor').value='#0f0e0d';
  $('hAccentColor').value='#e63950';
  $('hShowStats').checked=false;
  const statsEd=$('statsEditor'); if(statsEd) statsEd.innerHTML='';
  showToast('Hero form reset to defaults','info');
};
...
function updateImagePreviews(){
  ...
  [1,2,3].forEach(i=>{
    ...
    const field=$(`apImage${i}`);
    if(actions&&field) actions.style.display=field.value?'inline-flex':'none';
  });
}
let uploadTarget = 1; // 1, 2, or 3
function setUploadTarget(n){ uploadTarget = n; showToast(`Upload will fill Image ${n}`,'info'); }
function clearImageField(n){ const f=$(`apImage${n}`); if(f){ f.value=''; updateImagePreviews(); } }
...
// Duplicate / Clear helpers
let lastProductData = null;
function duplicateLastProduct(){
  if(!lastProductData){showToast('No previous product to duplicate','error');return;}
  const p=lastProductData;
  $('apName').value=p.name+' (Copy)';
  $('apCategory').value=p.category||'';
  $('apBrand').value=p.brand||'';
  $('apType').value=p.type||'New';
  $('apShortDesc').value=p.shortDesc||'';
  ...
  $('apSku').value=p.sku||'';
  $('apWeight').value=p.weight||'';
  $('apImage1').value=(p.images&&p.images[0])||'';
  $('apImage2').value=(p.images&&p.images[1])||'';
  $('apImage3').value=(p.images&&p.images[2])||'';
  $('apSizes').value=(p.sizes||[]).join(', ');
  $('apColors').value=(p.colors||[]).join(', ');
  ...
  $('apBadge').value=p.badge||'';
  $('apTags').value=(p.tags||[]).join(', ');
  const sr=$('specRows'); if(sr){ sr.innerHTML=''; if(p.specs) Object.entries(p.specs).forEach(([k,v])=>addSpecRow(k,v)); }
  updateCalcDisplay(); updateImagePreviews();
  showToast('Fields filled from last product','success');
}
...
$('apImageFile')?.addEventListener('change',()=>{
  ...
  reader.onload=e=>{
    ...
    // Put in selected target field
    $(`apImage${uploadTarget}`).value=b64;
    updateImagePreviews();
    showToast(`Image loaded into Image ${uploadTarget}`,'success');
  };
...
$('adminProductForm')?.addEventListener('submit',async e=>{
  ...
  try{
    ...
    // Store for duplicate feature
    lastProductData = {...productData, id: editingId || Date.now().toString()};
    cancelEdit();
...
function loadSettingsIntoForm(s){
  ...
  const storeName = s.storeName || '3mi Store';
  // Also update quick-edit label if exists
  const quickLabel = $('quickEditLabel');
  if (quickLabel) quickLabel.textContent = storeName;
  // Update admin sidebar logo
  const sidebarLogo = $('sidebarLogoTag');
  if (sidebarLogo) sidebarLogo.textContent = 'Admin';
  // Update loading screen logo
  const loadLogo = $('loadingLogo');
  if (loadLogo) loadLogo.textContent = storeName;
  // Update admin login logo
  const adminLoginLogo = $('adminLoginLogo');
  if (adminLoginLogo) adminLoginLogo.textContent = `${storeName} Admin`;
  // Update document title
  document.title = `Admin — ${storeName}`;
  $('sTagline').value=s.tagline||'';
  $('sDescription').value=s.description||'';
  $('sLogoUrl').value=s.logoUrl||'';
  $('sFaviconUrl').value=s.faviconUrl||'';
  $('sCopyright').value=s.copyright||'';
  $('sCurrency').value=s.currency||'Rs.';
  $('sCurrencyCode').value=s.currencyCode||'NPR';
  // Contact
  $('sWhatsapp').value=s.whatsapp||'';
  $('sPhone').value=s.phone||'';
  $('sEmail').value=s.email||'';
  $('sAddress').value=s.address||'';
  $('sBusinessHours').value=s.businessHours||'';
  ...
  $('sAnnouncementTextColor').value=s.announcementTextColor||'#ffffff';
  $('sAnnouncementLink').value=s.announcementLink||'';
  // Shipping
  ...
  $('sCodEnabled').checked=s.codEnabled!==false;
  $('sReturnPolicyUrl').value=s.returnPolicyUrl||'';
  // SEO
  $('sMetaTitle').value=s.metaTitle||'';
  $('sMetaDescription').value=s.metaDescription||'';
  $('sGaId').value=s.gaId||'';
  $('sFbPixelId').value=s.fbPixelId||'';
  $('sCustomHead').value=s.customHead||'';
  // Store Behavior
  $('sMaintenanceMode').checked=!!s.maintenanceMode;
  $('sMaintenanceMessage').value=s.maintenanceMessage||'';
...
$('saveAllSettingsBtn')?.addEventListener('click',()=>{
  const partial={
    ...
    announcementTextColor:$('sAnnouncementTextColor').value,
    announcementLink:$('sAnnouncementLink').value.trim(),
    // Shipping
    deliveryAreas:$('sDeliveryAreas').value.trim(),
    deliveryTime:$('sDeliveryTime').value.trim(),
    ...
    codEnabled:$('sCodEnabled').checked,
    returnPolicyUrl:$('sReturnPolicyUrl').value.trim(),
    // SEO
    metaTitle:$('sMetaTitle').value.trim(),
    metaDescription:$('sMetaDescription').value.trim(),
    gaId:$('sGaId').value.trim(),
    fbPixelId:$('sFbPixelId').value.trim(),
    customHead:$('sCustomHead').value.trim(),
    // Store Behavior
    maintenanceMode:$('sMaintenanceMode').checked,
    maintenanceMessage:$('sMaintenanceMessage').value.trim(),
...
// Keep individual save buttons for backward compat
$('saveIdentityBtn')?.addEventListener('click',()=>saveSetting({
  ...
  description:$('sDescription').value.trim(),faviconUrl:$('sFaviconUrl').value.trim(),
  currency:$('sCurrency').value.trim()||'Rs.',currencyCode:$('sCurrencyCode').value.trim()||'NPR',
}));
$('saveContactBtn')?.addEventListener('click',()=>saveSetting({
  ...
  businessHours:$('sBusinessHours').value.trim(),supportEmail:$('sSupportEmail').value.trim(),
}));
...
$('saveShippingBtn')?.addEventListener('click',()=>saveSetting({
  ...
  freeDeliveryMin:$('sFreeDeliveryMin').value?parseFloat($('sFreeDeliveryMin').value):null,
  deliveryCharge:$('sDeliveryCharge').value?parseFloat($('sDeliveryCharge').value):null,
  codEnabled:$('sCodEnabled').checked,returnPolicyUrl:$('sReturnPolicyUrl').value.trim(),
}));

// Maintenance mode toggle
$('sMaintenanceMode')?.addEventListener('change',toggleMaintenanceMessage);
function toggleMaintenanceMessage(){
  const show=$('sMaintenanceMode').checked;
  $('maintenanceMsgGroup').style.display=show?'block':'none';
}

// Preview & Reset
window.openStorePreview=()=>window.open('index.html','_blank');
window.resetAllSettings=()=>{
  if(confirm('Reset ALL settings to defaults? This cannot be undone.')){
    FB.setDoc(FB.doc(FB.db,'config','settings'),{
      storeName:'3mi Store',tagline:'Quality products, delivered.',
      ...
      accentColor:'#e63950',footerBg:'#0f0e0d',cardStyle:'default',gridCols:'4',
      stats:[{num:'500+',label:'Products'},{num:'50+',label:'Categories'},{num:'10k+',label:'Happy Customers'}]
    },{merge:true}).then(()=>showToast('Reset to defaults','info'));
  }
};
...
window.enableAllSocial=()=>{
  document.querySelectorAll('.social-item-editor input[data-field="enabled"]').forEach(cb=>cb.checked=true);
  showToast('All social links enabled','info');
};
window.disableAllSocial=()=>{
  document.querySelectorAll('.social-item-editor input[data-field="enabled"]').forEach(cb=>cb.checked=false);
  showToast('All social links disabled','info');
};
...
$('sidebarToggle')?.addEventListener('click',()=>{
  ...
  const ov=$('sidebarOverlay');if(ov)ov.classList.toggle('open',sb.classList.contains('open'));
});
...
function initQuickEditStore() {
  const container = $('quickEditStore');
  const label = $('quickEditLabel');
  const input = $('quickEditInput');
  ...
  input.addEventListener('blur', saveQuickEdit);
  ...
  function saveQuickEdit() {
    ...
    saveTimeout = setTimeout(async () => {
      try {
        await FB.setDoc(FB.doc(FB.db, 'config', 'settings'), { storeName: newName }, { merge: true });
        showToast('Store name updated', 'success');
        // Also update the store name in the Store Settings form
        $('sStoreName').value = newName;
      } catch (err) {
        showToast('Failed to save store name', 'error');
      }
    }, 500);
  }
}
